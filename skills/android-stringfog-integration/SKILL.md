...
description: Integrate, migrate, validate, or troubleshoot mobcoding StringFog encryption for an external Android AAR. Use when a user supplies an AAR path and needs its classes plus explicit application-source package roots encrypted, needs AGP 8/9 compatible StringFog configuration, wants to replace a legacy AAR StringFog plugin, or needs Release APK/AAB verification.
---
...
1. Inspect the AAR without modifying it.
   ...
   - Present the candidate AAR package roots and obtain confirmation before using them in `fogPackages`.
